module.exports = {
     ...
     */
    async patchAvatar(ctx, next) {
        let {name} = ctx.request.files.avatar

        const userService = ctx.state.services.user

        ctx.body = await userService.patchAvatar(ctx.state.user.id, name)
    },
    ...
    async getArticles(ctx, next) {
        ...
        const userService = ctx.state.services.user;
        ctx.body = await userService.getArticles(userId, page, limit);
    ...
    async getReplies(ctx, next) {
        ...
        ctx.body = await userService.getReplies(userId, page, limit);
    }
}
